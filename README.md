...
## 使用说明
1.目录设置
这个主要是设置下载的图片和nfo信息的位置,手动新建一个影视库专用的即可
STRM位置也是设置成这个文件夹,然后保存设置
2.网盘设置
首先需要扫描二维码登录,选择你想要的登录方式并获取二维码,115app去扫描二维码进行登录
其他的设置不用管
3.刮削
这里默认就好,后续会具体说明这些刮削方式
注意dmm和dmm2刮削必须挂上日本节点,否则无法刮削
4.字幕
实时效果不是很好,想用也可以用,
需要下载模型,模型需要挂节点才能下载,不然很慢
5.使用
这里115网盘登录以后,可以在网盘界面去选择你想要添加的视频,会自动使用dmm2刮削,然后在影片页面就能看看刚刚刮削的影片
(这里dmm2的刮削方式时候现在的新片,因为是从DMM中逆向出的接口,有高清图片的,老的影片没有数据)
也可以在网盘点击视频直接播放,支持VR视频播放,
播放VR视频时点击播放器右上角切换360,这里会启用陀螺仪的效果,转动手机,画面转动
